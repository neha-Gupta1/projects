...
```sh
make docker-build docker-push IMG=<some-registry>/volsnap:tag
```
...
```sh
make deploy IMG=<some-registry>/volsnap:tag
```
...
```sh
make build-installer IMG=<some-registry>/volsnap:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/volsnap/<tag or branch>/dist/install.yaml
```
